...
function BackToGame(){
    ...
    document.querySelector(".game-number").innerHTML = `#${
        Day
    }`;
    ...
    solution = [1, 3, 2, 2, 2, 2, 6, 1, 6, 2, 5, 0, 0, 3, 4, 4, 6, 1, 6, 3, 1, 1, 3, 0, 3]
    ...
    if(localStorage.LastResumeDate){
        ...
        if(dateDiffInDays(currentDate,date) == 0){
            ...
            document.querySelector(".moves-number").innerHTML = `${
                localStorage.TodaySwapCount
            }`;
        ...
        state = [2, 0, 3, 1, 3, 3, 6, 4, 6, 1, 4, 2, 1, 1, 5, 2, 6, 2, 6, 0, 3, 2, 1, 0, 3]
        playTime = 0;
        moves = 21;
        document.querySelector(".moves-number").innerHTML = `${
            21
...
function gameFinish(won, movesRemaining, state, timeSpent) {
    ...
    document.querySelectorAll("[data-field=games-played]").forEach(element => element.innerText = `${
        localStorage.GamesPlayed
    }`)
    document.querySelectorAll("[data-field=win-percent]").forEach(element => element.innerText = `${
        Math.round((freqArr[0] + freqArr[1] + freqArr[2] + freqArr[3] + freqArr[4] + freqArr[5])*100/localStorage.GamesPlayed)
    }`)
    document.querySelectorAll("[data-field=stars-count]").forEach(element => element.innerText = `${
        localStorage.StarsCollected
    }`)
    document.querySelectorAll("[data-field=current-streak]").forEach(element => element.innerText = `${
        localStorage.CurrentStreak
    }`)
    document.querySelectorAll("[data-field=highest-streak]").forEach(element => element.innerText = `${
        localStorage.HighestStreak
    }`)


    document.querySelector("[data-field=finish-moves]").innerHTML = `${
        21 - localStorage.TodaySwapCount
    }/21`;
    document.querySelector("[data-field=finish-time]").innerHTML = `${
        localStorage.TodayPlayTime
    }`;


    document.querySelectorAll(".stars-progress-count").forEach((element,index) => {element.innerText = `${
        freqArr[index]
    }`})


    document.querySelectorAll(".stars-progress-bar").forEach((element,index) => {element.style.width = `${
        Math.round(freqArr[index]*100/localStorage.GamesPlayed)
    }%`})
...
function makeTutorial(){
    ...
    document.querySelector(".visit-tomorrow").style.display = "none";
      ...
      var solution = [1, 3, 2, 2, 2, 2, 6, 1, 6, 2, 5, 0, 0, 3, 4, 4, 6, 1, 6, 3, 1, 1, 3, 0, 3];
      //state of that day
      var state = [2, 0, 3, 1, 3, 3, 6, 4, 6, 1, 4, 2, 1, 1, 5, 2, 6, 2, 6, 0, 3, 2, 1, 0, 3];
      ...
      if(localStorage.LastSolveDate){
        ...
        document.querySelectorAll("[data-field=win-percent]").forEach(element => element.innerText = `${
            Math.round(freqArr.reduce((partialSum, a) => partialSum + a, 0)*100/localStorage.GamesPlayed)
        }`)
        document.querySelectorAll("[data-field=stars-count]").forEach(element => element.innerText = `${
            localStorage.StarsCollected
        }`)
    ...
    boardElement.innerHTML = state
        .map((element, index) =>
            element === 6
                ...
                : `<div class="dice${
                      check(element, index, solution, state) === "success"
                          ? " no-drag"
                          : ""
